...
function ServiceDetail({ reverse, img, title, tagline, copy, bullets }) {
  return (
    <section
      className={`section-wrapper grid items-center gap-10 md:grid-cols-2 ${reverse ? "md:flex-row-reverse" : ""}`}
    >
      {/* Image */}
      <img
        src={img}
      ...
      {/* Text */}
      <div>
        <h2 className="mb-2 text-2xl font-bold text-dark">{title}</h2>
        <p className="mb-4 text-primary font-medium">{tagline}</p>
        <p className="mb-4 text-slate-600">{copy}</p>

        <ul className="space-y-2">
          {bullets.map((b) => (
            <li key={b} className="flex items-start gap-2">
              <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-accent" />
              <span className="text-sm text-dark/90">{b}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <section className="section-wrapper max-w-6xl">
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {[
            ...
            <div key={step} className="rounded-xl bg-white p-6 text-center shadow-sm">
              <p className="mb-2 text-3xl">{emoji}</p>
              <h3 className="mb-1 font-semibold text-dark">{step}</h3>
